local _tl_compat; if (tonumber((_VERSION or ''):match('[%d.]*$')) or 0) < 5.3 then local p, m = pcall(require, 'compat53.module'); if p then _tl_compat = m end end; local string = _tl_compat and _tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local _tl_table_unpack = unpack or table.unpack; local fmt = string.format
...
local function t_to_s(self, t)
   ...
   return string.char(_tl_table_unpack(r))
end
...
local ALPHABET_B256 = string.char(_tl_table_unpack(ascii))
...
local M = {
   ...
   from_b36 = from_b36,
   is_b36 = function(s) return from_b36:validate(s) end,
}
